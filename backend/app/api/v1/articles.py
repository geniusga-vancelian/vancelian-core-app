...
@router.get(
    ...
    description="List published articles linked to a specific offer. Requires USER role.",
)
async def get_offer_articles(
    ...
    # Get articles linked to this offer
    articles = db.query(Article).join(Article.offers).filter(
        Offer.id == offer_id,
        Article.status == ArticleStatus.PUBLISHED.value
    ).order_by(Article.published_at.desc().nullslast()).limit(limit).offset(offset).all()

...
@router.post(
    "/vaults/AVENIR/vesting/release",
    response_model=VestingReleaseSummaryResponse,
    summary="Release AVENIR vesting lots",
    description="Release mature AVENIR vesting lots. Idempotent and replayable. Requires ADMIN role.",
)
async def release_avenir_vesting(
    as_of: Optional[str] = Query(None, description="Date for maturity check (YYYY-MM-DD, default: today UTC)"),
    currency: str = Query("AED", description="Currency filter (default: AED)"),
    dry_run: bool = Query(False, description="Simulate without committing (default: false)"),
    http_request: Request = None,
    db: Session = Depends(get_db),
    principal: Principal = Depends(require_admin_role()),
) -> VestingReleaseSummaryResponse:
    ...
    try:
        ...
        logger.info(
            f"Releasing AVENIR vesting lots: as_of={as_of_date}, currency={currency}, dry_run={dry_run}",
            extra={"trace_id": trace_id, "as_of_date": str(as_of_date) if as_of_date else None, "currency": currency, "dry_run": dry_run}
        )
        ...
        summary = release_avenir_vesting_lots(
            ...
            currency=currency,
            dry_run=dry_run,
            trace_id=None,  # Will be generated by service
        )
        
        return VestingReleaseSummaryResponse(
            matured_found=summary['matured_found'],
        ...
        logger.exception(
            f"Unexpected error in release_avenir_vesting: {type(e).__name__}: {str(e)}",
            extra={"trace_id": trace_id}
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
